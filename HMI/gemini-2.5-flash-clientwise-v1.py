...
OUTPUT_EXCEL_FILE = '13_associates_data_with_client_3months_analysis_clientwise.xlsx' # Name for the output Excel file
...
def main():
    ...
    # --- 3. Get Unique Client Names ---
    # Get all unique client names from the specified column, drop any NaN values
    unique_clients = df[CLIENT_NAME_COLUMN].dropna().unique().tolist()
    ...
    print(f"\nFound {len(unique_clients)} unique client to analyze: {unique_clients}")
    ...
    for client_name in unique_clients:
        ...
        client_df = df[df[CLIENT_NAME_COLUMN] == client_name].copy() # Use .copy() to avoid SettingWithCopyWarning
        ...
        if not all_combined_text.strip(): # Check if the combined text is just whitespace
            print(f"No combined Summary/Description found for client: {client_name}")
            results_list.append({
                'Client Name': client_name,
        ...
        print(f"Found {len(client_df)} relevant rows for {client_name}.")

        # --- 5. Analyze with Gemini ---
        analysis_result = analyze_text_with_gemini(all_combined_text, API_KEY, GEMINI_MODEL)

        # Append results to the list
        result_entry = {'Client Name': client_name}
        if analysis_result:
