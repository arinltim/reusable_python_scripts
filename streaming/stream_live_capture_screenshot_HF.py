...
print("Loading Hugging Face VQA model on CPU...")
try:
    # Use the VQA pipeline instead of image-classification
    ...
    print(f"Failed to load Hugging Face VQA model: {e}")
    vqa_pipeline = None


# --- Your realstream and analyze_frame functions (no change needed) ---
...
def analyze_frame_with_vqa_prompt(image_data: bytes) -> bool:
    """
    Analyzes an image using a local VQA model and a text prompt.
    """
    if not vqa_pipeline:
        print("VQA pipeline not available. Skipping analysis.")
        return False
    ...
    try:
        ...
        question = "Is there a weather map or weather information displayed in this image?"

        # Get the answer from the VQA pipeline
        results = vqa_pipeline(image=image, question=question, top_k=1)

        # The result is a list of dictionaries, e.g., [{'score': 0.98, 'answer': 'yes'}]
        if results:
            top_answer = results[0]
            # Check if the model answered "yes" with a reasonable confidence score
            if "yes" in top_answer['answer'].lower() and top_answer['score'] > 0.5:
                print(f"[VQA] Positive detection: Answer='{top_answer['answer']}', Score={top_answer['score']:.2f}")
                return True

        return False

    except Exception as e:
        print(f"Error analyzing image with Hugging Face VQA model: {e}")
        return False
        ...
        while not stop_event.is_set():
            ...
            while True:
                ...
                if jpeg_data:
                    ...
                    if analyze_frame_with_vqa_prompt(jpeg_data):
                        frame_count += 1
