...
class StructureConfigGenerator:
    ...
    def _build_bronze_rules_json(self, data, parent_path=""):
        """ (UPDATED) Recursively builds BRONZE rules as a versioned dictionary. """
        rules = {}
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                if isinstance(value, dict) and value:
                    ...
                    rules.update(self._build_bronze_rules_json(value[0], current_path))
                else:
    ...
    def _build_silver_rules_json(self, data, parent_path=""):
        """ (UPDATED) Recursively builds SILVER rules as a versioned dictionary. """
        rules = {}
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                if isinstance(value, dict) and value:
                    ...
                    rules.update(self._build_silver_rules_json(value[0], bronze_target_name))
                else:
                    ...
                    elif isinstance(value, list) and not (value and isinstance(value[0], dict)):
    ...
    def _build_rules_from_dataframe(self, df):
        """ (UPDATED) Builds Bronze and Silver rules from a DataFrame as versioned dictionaries. """
        bronze_rules_dict = {}
        silver_rules_dict = {}
        ...
        for i, col in enumerate(df.columns):
            ...
            # Bronze rule: path is the 1-based index, target is UPPERCASE column name
            ...
            # Silver rule: path and target are UPPERCASE column name, type is inferred
            silver_rule = {
                "path": target_name,
                "target_name": target_name,
                "type": self._map_pandas_dtype_to_silver(df[col].dtype, col)
            }
    ...
    def generate(self, file_path):
        ...
        # Define standard audit fields in the new versioned dictionary format
        silver_audit_fields = {
            ...
            if extension == '.json':
                ...
                if all(isinstance(v, dict) for v in input_data.values()):
                    for sub_source_id, data_object in input_data.items():
                        ...
                        final_config["structure_config"][sub_source_id] = {"bronze": bronze_rules, "silver": silver_rules}
                    return final_config
                else:
        ...
        silver_rules.update(silver_audit_fields)
        final_config["structure_config"][source_id] = {
            "bronze": bronze_rules,
            "silver": silver_rules
        }
        return final_config
